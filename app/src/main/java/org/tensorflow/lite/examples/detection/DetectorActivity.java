...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  /**new
   * Variables agregadas
   * mascarilla -> para ajustar el tamaño (mascara.png) con respecto a la resolución de la pantalla.
   * leyenda -> muestra el estado de detección (GREEN,RED and BLUE)
   * contadorRed -> Tiempo de detección SIN mascarilla
   * contadorGreen -> Tiempo de detección CON mascarilla
   * contadorAll -> Tiempo sin actividad.
   */
  static ImageView mascarilla;

  static TextView leyenda;
  Integer contadorRed=0,contadorGreen=0,contadorAll=0;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mp = MediaPlayer.create(getApplicationContext(), R.raw.pls_wear_mask);
...
// since is smaller it can speed up the detection process.
  ...
  protected void processImage() {
    ...
    /**
     * Ajustando tamaño de mascarilla con respecto a la resolución de la pantalla
     */
    ancho = (getResources().getDisplayMetrics().widthPixels +15)/2;
    alto = (getResources().getDisplayMetrics().heightPixels)/2 ;
    mascarilla = findViewById(R.id.imageView2);
    android.view.ViewGroup.LayoutParams layoutParams = mascarilla.getLayoutParams();
    layoutParams.width = ancho;
    layoutParams.height = alto;
    mascarilla.setLayoutParams(layoutParams);
    ...
    if(contadorAll>15){
      ...
      runOnUiThread(new Runnable() {
        @Override
        public void run() {
          leyenda = findViewById(R.id.textView);
          leyenda.setText("¡Bienvenido!");
          leyenda.setBackgroundColor(Color.BLUE);
          leyenda.setTextColor(Color.WHITE);
        }
      });
    }
    ++timestamp;
  ...
  @Override
  protected void setVoiceOption(Boolean voice_on) {
    runInBackground(() -> detector.setVoiceOption(voice_on));
  }
      ...
      if (boundingBox != null && goodConfidence) {
        ...
        if (resultsAux.size() > 0) {
          ...
          if (conf >= 0.6f) {
            ...
            /******Contador y tiempos de Detección*******/
            if(contadorGreen>5){
              contadorRed=0;
              contadorGreen=0;
              contadorAll=0;
              runOnUiThread(new Runnable() {
                ...
                public void run() {
                  leyenda = findViewById(R.id.textView);
                  leyenda.setText("Mascara Detectada");
                  leyenda.setBackgroundColor(Color.GREEN);
                  leyenda.setVisibility(View.VISIBLE);
                  leyenda.setTextColor(Color.BLACK);

                }
              });
            }else if(contadorRed>5){
              contadorRed=0;
              contadorGreen=0;
              contadorAll=0;
              runOnUiThread(new Runnable() {
                ...
                public void run() {
                  leyenda = findViewById(R.id.textView);
                  leyenda.setText("Mascara NO Detectada");
                  leyenda.setBackgroundColor(Color.RED);
                  leyenda.setVisibility(View.VISIBLE);
                  leyenda.setTextColor(Color.WHITE);
        ...
        Switch voice_switch = (Switch)findViewById(R.id.voice_switch);
        Boolean voice_on = voice_switch.isChecked();

        if(!playing && !start_play && color == Color.RED && voice_on) {
          start_play = true;
          //int ms = mp.getDuration();
          mp.start(); //play: please wear mask.
          LOGGER.i("pls wear face mask.");
        }
        if(playing)
          start_play = false;
